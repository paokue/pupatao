...
async function upsertAdmin2() {
  const email = "admin2@pupatao.com";
  const passwordHash = await bcrypt.hash("admin1234", BCRYPT_ROUNDS);
  const admin = await prisma.admin.upsert({
    where: { email },
    update: {}, // don't overwrite an existing admin's password on re-seed
...
async function backfillExistingUsers() {
  ...
  if (backfilledCodes > 0)
    console.log(`✓ backfilled referralCode on ${backfilledCodes} user(s)`);
  ...
  for (const u of allUsers) {
    ...
    if (!promo) {
      await prisma.wallet.create({
        data: { userId: u.id, type: WalletType.PROMO, balance: 0 },
      });
      backfilledPromo++;
    }
  }
  if (backfilledPromo > 0)
    console.log(`✓ backfilled PROMO wallet on ${backfilledPromo} user(s)`);
